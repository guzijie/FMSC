...
#  --config_env configs/env.yml --config_exp configs/pretext/simclr_cifar10.yml

def main():
    args.config_env = 'configs/env.yml'
    args.config_exp = 'configs/pretext/simclr_cifar10.yml'

    # Retrieve config file  这个函数return cfg字典了
    ...
    train_transforms = get_train_transformations(p)#train数据集用simclr的图像增广
    ...
    # Memory Bank #memory bank只是参与knn近邻的验证，并不参与训练
    ...
    criterion = get_criterion(p)   # 这里的criterion就是做loss criterion = SimCLRLoss(**p['criterion_kwargs']) temperature: 0.1
    print('Criterion is {}'.format(criterion.__class__.__name__))
    criterion = criterion.cuda()

    # Optimizer and scheduler
    print(colored('Retrieve optimizer', 'blue'))
    optimizer = get_optimizer(p, model)  #optimizer的相关知识
    print(optimizer)

    state = torch.load(p['pretext_model'], map_location='cpu') # 把预训练模型pretext_model放到'./results/cifar-10/pretext/model.pth.tar'里去
    # 改了以后，应该是要重新训练模型的，我是加在聚类模型上，所以预训练的对比学习模型应该不用动
