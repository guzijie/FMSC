...
class CIFAR10(Dataset):
    ...
    filename = "wnx/cifar-10-python.tar.gz"
    ...
    def __init__(self, root=MyPath.db_root_dir('cifar-10'), train=True, transform=None, 
        ...
        self.train = train  # training set or test set 这时候传进来的train是false，说明要eval是测试集。
        self.classes = ['plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']

        if download:
            self.download()

        if not self._check_integrity():  #跳到这里的bug说明self._check_integrity()是false了 执行这里的函数
            raise RuntimeError('Dataset not found or corrupted.' +
                               ' You can use download=True to download it')

        if self.train:  #train=false表示是eval测试的时候，test的时候，最终写到论文表格上的那个数据
    ...
    def _check_integrity(self):  #这里的_check_integrity()为什么是false的
        root = self.root   #root=MyPath.db_root_dir('cifar-10')  if database == 'cifar-10': return '/data/wnx/'
        for fentry in (self.train_list + self.test_list):
            filename, md5 = fentry[0], fentry[1]
            fpath = os.path.join(root, self.base_folder, filename)  #base_folder = 'cifar-10-batches-py'  /data/wnx/cifar-10-batches-py/data_batch_1到5加上meta
            if not check_integrity(fpath, md5):  #utils.py里的check_integrity   返回了false
                return False
        return True
