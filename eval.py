...
FLAGS.add_argument('--config_exp', help='Location of config file')   #是scan_cifar10.yml
...
def main():
    ...
    dataloader = get_val_dataloader(config, dataset)  # 就等于=torch.utils.data.DataLoader(dataset)
    print('Number of samples: {}'.format(len(dataset))) # Number of samples: 10000 说明没拿错，拿的就是test_batch的数据集
    ...
    if config['setup'] in ['simclr', 'moco']:
        ...
        for topk in [1, 5, 20]: # Similar to Fig 2 in paper 
            _, acc = memory_bank.mine_nearest_neighbors(topk)  #只是直接从memorybank里 取出来?
        ...
        clustering_stats = hungarian_evaluate(head, predictions, dataset.classes, compute_confusion_matrix=True,
                                              confusion_matrix_file='./results/'+config['train_db_name']+'/'+config['train_db_name']+ '_confusion_matrix.pdf')
        # config['train_db_name']即cifar-10，    ./cifar-10/cifar-10_confusion_matrix.png
...
def visualize_indices(indices, dataset, hungarian_match, config):
    ...
    for c in range(indices.shape[0]) :
        ...
        for idx in indices_c:
            ...
            plt.axis('off')
            plt.imshow(img)
            #plt.show()   # 原来是plt.savefig('./'+config['train_db_name']
            plt.savefig('./results/'+config['train_db_name']+'/'+config['train_db_name']+'_'+str(c)+'_'+str(idx)+'.png', dpi=500, bbox_inches='tight')
            # format="svg"
