...
def get_criterion(p):
    if p['criterion'] == 'simclr':
        ...
        criterion = SCANLoss(**p['criterion_kwargs']) # criterion_kwargs: entropy_weight:5.0 #这句就是criterion = SCANLoss(entropy_weight=5.0)
...
def get_model(p, pretrain_path=None):
    #pretrain_pth是'../Pre-Trained Models'吗 不是，放在results文件里
    # Get backbone
    if p['backbone'] == 'resnet18':
    ...
    if p['setup'] in ['simclr', 'moco']:
        ...
        model = ContrastiveModel(backbone, **p['model_kwargs']) # 不管是ContrastiveModel 还是 ClusteringModel ，其backbone都是一样的

    elif p['setup'] in ['scan', 'selflabel']:
        from models.models import ClusteringModel
        if p['setup'] == 'selflabel':
            assert(p['num_heads'] == 1) # scan_cifar10.yml里定义了
        model = ClusteringModel(backbone, p['num_classes'], p['num_heads']) # num_heads: 3
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'cifar-10':
        ...
        dataset = CIFAR10(train=True, transform=transform, download=False)#True
