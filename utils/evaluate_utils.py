...
def get_predictions(p, dataloader, model, return_features=False):
    ...
    model.eval()
    predictions = [[] for _ in range(p['num_heads'])]  # scan_cifar_10.yml里num_heads: 3
    probs = [[] for _ in range(p['num_heads'])] #放3个[[], [], []]
    targets = []
    if return_features:
        ft_dim = get_feature_dimensions_backbone(p) #if p['backbone'] == 'resnet18': return 512
        features = torch.zeros((len(dataloader.sampler), ft_dim)).cuda() #tensor(100000,512)全0向量 CIFAR10总共10类，每类6000张，共6万张：5万用于训练，5个批次，每一批1万张；1万用来测试，单独一批次
    ...
    else:#说明dataloader.dataset不是NeighborsDataset这个类型的数据集，这个数据集还是自己定义的 在data/custom_datatset.py里定义了
        key_ = 'image'
        include_neighbors = False

    ptr = 0 # train的时候有,_ eval的时候没有
    # print('dataloader:',dataloader) #,_ 会是dataloader出问题吗？应该不会啊，之前的dataloader不也是这个吗 真是服了，TMD，去掉batch,_就可以了，直接用batch
    for batch,_ in dataloader:#,_ , _ j, #too many values to unpack (expected 2)这里的dataloader是common_config.py里的 torch.utils.data.DataLoader(dataset, num_workers=p['num_workers'],batch_size=p['batch_size'], shuffle=False)
        # print(batch)#---------------------------------------------那要看batch里到底是什么东西，要看里面是不是有['image']这个索引，总不可能是dataloader datatset拿错了吧，没拿到test_batch??
        images = batch[key_].cuda(non_blocking=True) #这句不就是batch['image'].cuda(non_blocking=True)吗 TypeError: string indices must be integers 但这个key_就是'image'啊
        bs = images.shape[0]
        ...
        for i, output_i in enumerate(output):
            predictions[i].append(torch.argmax(output_i, dim=1)) # argmax 返回指定维度最大值的序号 dim=0每一列 dim=1每一行
...
def scan_evaluate(predictions):
    ...
    for head in predictions:
        ...
        neighbors = neighbors.contiguous().view(-1) # 而通过contiguous让其变得连续后就可以使用view #x.view(-1)##拉平，观察底层数据
        ...
        # Total loss
        total_loss = - entropy_loss + consistency_loss + ce_loss  # 文章里说是loss_total=global_consis+local_consis+ global_class+local_class+lamda*entropy
...
def hungarian_evaluate(subhead_index, all_predictions, class_names=None, 
                        ...
                        confusion_matrix_file=None): # confusion_matrix_file就是 ./cifar-10/cifar-10_confusion_matrix.png
